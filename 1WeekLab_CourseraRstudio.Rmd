...
arbuthnot <- arbuthnot %>%
  mutate(boys_Proportion = boys/(boys + girls))
ggplot(data = arbuthnot, aes(x = year, y = boys_Proportion)) +
  geom_line() +
  geom_point()
```
...
present <- present %>% 
  mutate(total = boys + girls, proportion = boys / total)
ggplot(data = present, aes(x= year, y= proportion))+
  geom_line()+
  geom_point()
```
...
#present <- present %>%
#  mutate(highest_year =  year(arrange(desc(total))
present <- arrange(present, desc(present$total))
present
```
